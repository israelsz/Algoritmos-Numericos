function [iteraciones,errores,raices,erroresEuclideas] = newtonMultiEuclidea(F,tolerancia,valoresExactos)
syms x y z
iteraciones = [];
raices = [];
errores = [];
...
for i = 1: 3
    sumatoria = sumatoria + (abs(vectorErrorAbs(i)))^2;
end
sumatoria = sqrt(sumatoria);
errorEucledia = sumatoria;
%erroresEuclideas = [erroresEuclideas, errorEucledia];
sumatoria = 0;

while errorEucledia > tolerancia
    ...
    %Actualizacion error Norma Euclídea
